...
## Sticky MAC Troubleshooting and Validation

### Issue Identified

Sticky MAC addresses were not being learned on secured interfaces. Initial verification showed that port security was enabled, but no MAC addresses had been learned.

![Initial Observation](Screenshots/23_NetworkSecurity_StickyMAC_Initial_Observation.png)

### Interface Verification

Interface status was reviewed to confirm that all configured ports were operational.
...
The MAC address table was inspected and found to be empty.

![Empty MAC Table](Screenshots/25_NetworkSecurity_StickyMAC_Empty_MAC_Table.png)

### Resolution

Traffic was generated between connected workstations using ICMP ping commands to trigger MAC learning.

![Traffic Generation](Screenshots/26_NetworkSecurity_StickyMAC_Traffic_Generation.png)

### Validation

After traffic was generated, MAC addresses appeared in the MAC address table and were learned as Sticky MAC entries.
...
* Entries were learned as Sticky MAC bindings.
* Sticky MAC count increased to 1 on all secured ports.
* Port security remained active with no violations recorded.

### Lessons Learned

This issue reinforced an important troubleshooting concept: port security can be configured correctly and still show zero learned MAC addresses if no traffic has traversed the interface. Sticky MAC learning requires actual network traffic before the switch can dynamically learn and bind a MAC address to a secured port.
